...
BACKPRESSURE = Counter(
    "websocket_backpressure_disconnects_total", "Backpressure disconnects"
)
CROSS_SCOPE = Counter("websocket_cross_scope_denials_total", "Cross-scope denials")
LATENCY = Histogram(
    "websocket_delivery_latency_seconds", "Persistence-to-socket delivery latency"
)
...
def log_event(message: str, **fields: Any) -> None:
    safe = {
        ...
        if k not in {"ticket", "jwt", "token", "phone", "customer_name"}
    }
    LOG.info(
        json.dumps(
            {"timestamp": now().isoformat(), "message": message, **safe},
            separators=(",", ":"),
            default=str,
        )
    )
...
class Settings:
    ...
    jwks_url = os.getenv(
        "KEYCLOAK_JWKS_URL",
        "http://keycloak:8080/realms/codestra/protocol/openid-connect/certs",
    )
    ...
    allowed_origins = frozenset(
        filter(
            ...
            os.getenv(
                "ALLOWED_ORIGINS",
                "https://crm.codestra.agency,https://phone.codestra.agency",
            ).split(","),
        )
    )
    ...
    max_message_size = min(
        262144, max(1024, int(os.getenv("MAX_MESSAGE_SIZE", "65536")))
    )
    ...
    event_source_health_url = os.getenv(
        "INTERNAL_EVENT_SOURCE_HEALTH_URL", "http://middleware:8095/health"
    )
...
async def verify_schema(pool: asyncpg.Pool) -> None:
    ...
    if head != DATABASE_MIGRATION_HEAD:
        raise RuntimeError(
            f"database migration head mismatch: expected {DATABASE_MIGRATION_HEAD}"
        )
...
async def lifespan(app: FastAPI):
    ...
    app.state.pool = await asyncpg.create_pool(
        settings.database_url, min_size=2, max_size=20, command_timeout=5
    )
...
def roles_of(claims: dict[str, Any]) -> set[str]:
    ...
    client = set(
        claims.get("resource_access", {}).get(settings.audience, {}).get("roles", [])
    )
    return realm | client
...
def decode_access_token(request: Request) -> dict[str, Any]:
    ...
    if any(not claims.get(field) for field in required) or not isinstance(
        claims["campaigns"], list
    ):
...
async def readyz(request: Request) -> dict[str, Any]:
    checks: dict[str, bool] = {
        "application": True,
        "authentication_configuration": bool(
            settings.issuer and settings.audience and settings.jwks_url
        ),
    }
    try:
...
async def create_session(
    request: Request, claims: dict[str, Any] = Depends(decode_access_token)
) -> dict[str, Any]:
    ...
    session_expires = min(
        datetime.fromtimestamp(claims["exp"], timezone.utc), now() + timedelta(hours=8)
    )
    async with request.app.state.pool.acquire() as connection, connection.transaction():
        await connection.execute(
          ...
          VALUES($1,$2,$3,$4,$5,$6,$7,$8::jsonb,$9,$10)""",
            ...
            claims["sub"],
            claims["tenant_id"],
            claims["business_unit_id"],
            claims["agent_id"],
            claims["vicidial_user"],
            str(claims["extension"]),
            json.dumps(claims["campaigns"]),
            now(),
            session_expires,
        )
        await connection.execute(
            "INSERT INTO realtime_tickets(ticket_hash,session_id,expires_at) VALUES($1,$2,$3)",
            ticket_hash,
            session_id,
            expires_at,
        )
    log_event(
        "realtime_session_issued",
...
async def consume_ticket(pool: asyncpg.Pool, ticket: str) -> dict[str, Any] | None:
    ...
    async with pool.acquire() as connection, connection.transaction():
        ...
        await connection.execute(
            "UPDATE realtime_sessions SET active_connection=true WHERE session_id=$1",
            session["session_id"],
        )
        result = dict(session)
        stored_campaigns = result["campaigns"]
        result["campaigns"] = (
            json.loads(stored_campaigns)
            if isinstance(stored_campaigns, str)
            else list(stored_campaigns)
        )
        return result


async def replay(
    pool: asyncpg.Pool, scope: dict[str, Any], last_event_id: str | None
) -> list[dict[str, Any]]:
    after = 0
    if last_event_id:
        value = await pool.fetchval(
            "SELECT ordinal FROM realtime_events WHERE event_id=$1", last_event_id
        )
        after = int(value or 0)
    rows = await pool.fetch(
        """SELECT event_id,schema_version,event_type AS type,correlation_id,occurred_at AS timestamp,
    ...
    try:
        try:
            frame = await asyncio.wait_for(
                websocket.receive_json(), settings.auth_timeout
            )
        ...
        missing = (
            await replay(websocket.app.state.pool, scope, last_event_id)
            if last_event_id
            else []
        )
        if last_event_id:
        ...
        await websocket.send_json(
            {"type": "authenticated", "session_id": sid, "replayed": len(missing)}
        )
        ...
        while True:
            ...
            if message.get("type") == "disconnect":
                ...
                return
            await websocket.send_json(
                {"type": "error", "code": "client_command_not_allowed"}
            )
    except WebSocketDisconnect:
        pass
    finally:
        if scope:
            await websocket.app.state.pool.execute(
                "UPDATE realtime_sessions SET active_connection=false WHERE session_id=$1",
                scope["session_id"],
            )
...
async def publish_event(event: Event, request: Request) -> dict[str, Any]:
    ...
    row = await request.app.state.pool.fetchrow(
      ...
      ON CONFLICT(event_id) DO NOTHING RETURNING ordinal""",
        event.event_id,
...
@app.post(
    "/internal/v1/realtime/sessions/{session_id}/revoke",
    dependencies=[Depends(require_internal)],
)
async def revoke_session(session_id: uuid.UUID, request: Request) -> dict[str, bool]:
    changed = await request.app.state.pool.fetchval(
        "UPDATE realtime_sessions SET revoked_at=now() WHERE session_id=$1 AND revoked_at IS NULL RETURNING true",
        session_id,
    )
