"""Grant the runtime role least-privilege canonical registry access.

Revision ID: 0018_campaign_registry_grants
...
def upgrade() -> None:
    ...
    statements.append(
        f"GRANT USAGE, SELECT ON SEQUENCE campaign_identity_global_seq "
        f"TO {RUNTIME_ROLE};"
    )
    _when_role_exists("\n".join(statements))
...
def downgrade() -> None:
    ...
    statements.append(
        f"REVOKE ALL PRIVILEGES ON SEQUENCE campaign_identity_global_seq "
        f"FROM {RUNTIME_ROLE};"
    )
    _when_role_exists("\n".join(statements))
