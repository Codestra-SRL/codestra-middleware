...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column("parent_campaign_number", sa.Integer(), sa.ForeignKey("campaign_registry.campaign_number")),
        sa.Column(
        ...
        sa.Column("registry_status", sa.String(32), nullable=False, server_default="PROPOSED_DISABLED"),
        sa.Column("policy_hash", sa.String(64), nullable=False),
        sa.Column("source_change_id", sa.String(128), nullable=False),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.CheckConstraint("campaign_number > 0 AND campaign_number % 100 = 0", name="ck_campaign_registry_number"),
        sa.CheckConstraint("campaign_code ~ '^[A-Z]{3}$'", name="ck_campaign_registry_code"),
        sa.CheckConstraint(
    ...
    op.create_table(
        ...
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column("campaign_number", sa.Integer(), sa.ForeignKey("campaign_registry.campaign_number"), nullable=False),
        ...
        sa.Column("identity_state", sa.String(24), nullable=False, server_default="ID_ASSIGNED"),
        sa.Column("dialing_state", sa.String(24), nullable=False, server_default="NOT_ELIGIBLE"),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.UniqueConstraint("campaign_number", "identity_type", "sequence_value", name="uq_campaign_object_identity_sequence"),
        sa.UniqueConstraint("source_system", "source_object_id", "identity_type", name="uq_campaign_object_identity_source"),
        sa.CheckConstraint(
    ...
    op.create_index("ix_campaign_object_identity_campaign", "campaign_object_identity", ["campaign_number", "identity_type"])
    op.create_table(
        "campaign_search_alias",
        sa.Column("alias", sa.String(160), primary_key=True),
        sa.Column("campaign_number", sa.Integer(), sa.ForeignKey("campaign_registry.campaign_number"), nullable=False),
        sa.Column("object_identity_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("campaign_object_identity.id"), nullable=True),
        sa.Column("alias_type", sa.String(32), nullable=False),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
    )
    op.create_index("ix_campaign_search_alias_scope", "campaign_search_alias", ["campaign_number", "alias"])
    op.create_table(
        "campaign_feature_gate",
        sa.Column("campaign_number", sa.Integer(), sa.ForeignKey("campaign_registry.campaign_number"), primary_key=True),
        ...
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.CheckConstraint("status IN ('DISABLED','REVIEW','APPROVED','ACTIVE','PAUSED')", name="ck_campaign_feature_gate_status"),
    )
    op.create_table(
    ...
    for table in ("campaign_registry", "campaign_object_identity", "campaign_search_alias", "campaign_activation_audit"):
        op.execute(
...
def downgrade():
    ...
    for table in ("campaign_activation_audit", "campaign_search_alias", "campaign_object_identity", "campaign_registry"):
        op.execute(f"DROP TRIGGER trg_{table}_no_delete ON {table}")
    op.execute("DROP TRIGGER trg_campaign_activation_audit_update ON campaign_activation_audit")
    op.execute("DROP TRIGGER trg_campaign_search_alias_update ON campaign_search_alias")
    op.execute("DROP TRIGGER trg_campaign_object_identity_update ON campaign_object_identity")
    op.execute("DROP TRIGGER trg_campaign_registry_identity_update ON campaign_registry")
    ...
    op.drop_index("ix_campaign_object_identity_campaign", table_name="campaign_object_identity")
