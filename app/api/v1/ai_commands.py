...
async def submit_command(command: AICommand, subject: Tenant = Depends(tenant),
                         db: AsyncSession = Depends(get_session)) -> dict[str, Any]:
    if not settings.ai_orchestration_enabled:
        raise HTTPException(503, "AI orchestration is disabled")
    if command.tenant_id != subject.organization_id or command.actor_id != subject.user_id:
...
async def get_command(command_id: UUID, subject: Tenant = Depends(tenant),
                      db: AsyncSession = Depends(get_session)) -> dict[str, Any]:
    try:
        return await ai_orchestration.get(db, command_id, subject.organization_id,
...
async def cancel_command(command_id: UUID, subject: Tenant = Depends(tenant),
                         correlation_id: str = Depends(correlation),
                         db: AsyncSession = Depends(get_session)) -> dict[str, Any]:
    try:
        state = await ai_orchestration.cancel(db, command_id, subject.organization_id,
                                              subject.workspace_id, subject.user_id,
                                              correlation_id)
    except LookupError as exc:
...
async def get_result(command_id: UUID, subject: Tenant = Depends(tenant),
                     db: AsyncSession = Depends(get_session)) -> dict[str, Any]:
    try:
        return await ai_orchestration.result(db, command_id, subject.organization_id,
                                             subject.workspace_id)
    except LookupError as exc:
...
async def _decision(command_id: UUID, body: Decision, subject: Tenant,
                    correlation_id: str, db: AsyncSession, approved: bool) -> dict[str, Any]:
    ...
    try:
        state = await ai_orchestration.decide(
            ...
            subject.user_id, correlation_id, approved, body.reason,
        )
    except LookupError as exc:
...
async def approve(command_id: UUID, body: Decision, subject: Tenant = Depends(tenant),
                  correlation_id: str = Depends(correlation),
                  db: AsyncSession = Depends(get_session)) -> dict[str, Any]:
...
async def reject(command_id: UUID, body: Decision, subject: Tenant = Depends(tenant),
                 correlation_id: str = Depends(correlation),
                 db: AsyncSession = Depends(get_session)) -> dict[str, Any]:
...
async def capabilities(_: Tenant = Depends(tenant),
                       db: AsyncSession = Depends(get_session)) -> dict[str, Any]:
    rows = (await db.execute(text("""SELECT profile,command_types,logical_model,provider,
      enabled,max_input_tokens,max_output_tokens FROM ai_model_capabilities ORDER BY profile"""))).mappings().all()
    return {"capabilities": [dict(row) for row in rows], "external_endpoints_exposed": False}


@router.get("/usage")
async def usage(subject: Tenant = Depends(tenant),
                db: AsyncSession = Depends(get_session)) -> dict[str, Any]:
    row = (await db.execute(text("""SELECT COALESCE(sum(tokens),0) tokens,
      ...
      {"tenant": subject.organization_id, "workspace": subject.workspace_id})).mappings().one()
    return {"tenant_id": subject.organization_id, "workspace_id": subject.workspace_id,
            "date": "current", **dict(row)}
