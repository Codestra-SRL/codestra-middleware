...
class TelephonyServiceClient:
    ...
    async def dispatch(
        ...
        try:
            ...
            # Transmission may have reached the adapter.  Persist an ambiguous
            # operation keyed by the stable command identity and move directly
            # to target readback; never repeat the mutation blindly.
            return {
                "operation_id": command_id,
                "adapter_identity": str(route.endpoint_id),
                "target_public_id": command_target_public_id(command),
                "environment": command.environment,
