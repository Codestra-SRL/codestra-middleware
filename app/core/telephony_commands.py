...
def normalized_actual_state(
    ...
    if not isinstance(candidate, dict):
        raise TypeError("readback desired_state must be an object")
    expected = command.desired_state()
    return {key: candidate.get(key) for key in expected}


def command_target_public_id(command: TelephonyCommandRequest) -> str:
    payload = command.payload
    by_type = {
        TelephonyCommandType.USER_APPLY: payload.agent_public_id,
        TelephonyCommandType.USER_DISABLE: payload.agent_public_id,
        TelephonyCommandType.PHONE_APPLY: payload.phone_public_id,
        TelephonyCommandType.PHONE_DISABLE: payload.phone_public_id,
        TelephonyCommandType.ENDPOINT_APPLY: payload.endpoint_public_id,
        TelephonyCommandType.ENDPOINT_DISABLE: payload.endpoint_public_id,
        TelephonyCommandType.CONTACT_REVOKE: payload.endpoint_public_id,
        TelephonyCommandType.CONTACTS_REVOKE_ALL: payload.endpoint_public_id,
    }
    return by_type.get(command.command_type) or command.aggregate_public_id
