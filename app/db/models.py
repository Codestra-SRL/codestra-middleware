...
class TelephonyOperationJournal(Base):
    ...
    state: Mapped[str] = mapped_column(String(32), nullable=False)
    adapter_identity: Mapped[str] = mapped_column(String(144), nullable=False)
    target_public_id: Mapped[str] = mapped_column(String(144), nullable=False)
    environment: Mapped[str] = mapped_column(String(16), nullable=False)
    desired_state_version: Mapped[int] = mapped_column(Integer, nullable=False)
    ...
    __table_args__ = (
        CheckConstraint(
            "environment IN ('staging','test','production')",
            name="ck_telephony_operation_environment",
        ),
        CheckConstraint(
            "desired_state_version >= 1",
            name="ck_telephony_operation_desired_version",
        ),
        CheckConstraint(
...
class TelephonyOperationTransition(Base):
    ...
    __table_args__ = (
        CheckConstraint("sequence >= 1", name="ck_telephony_transition_sequence"),
        UniqueConstraint(
            "operation_id",
            "sequence",
            name="uq_telephony_transition_operation_sequence",
        ),
        UniqueConstraint(
            "operation_id",
            "idempotency_key",
            name="uq_telephony_transition_idempotency",
        ),
        Index("ix_telephony_transition_correlation", "correlation_id"),
    )
...
class TelephonyTerminalResult(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "environment IN ('staging','test','production')",
            name="ck_telephony_result_environment",
        ),
        CheckConstraint(
            "requested_state_version >= 1 AND applied_state_version >= 1 "
            "AND observed_state_version >= 1",
            name="ck_telephony_result_versions",
        ),
        UniqueConstraint(
