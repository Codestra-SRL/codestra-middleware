...
from app.api.v1.ai_workflows import router as ai_workflows_router
...
AI_WORKFLOW_JWT_PATH = re.compile(r"^/api/v1/ai-workforce(?:/|$)")
...
async def control_request_guard(request: Request, call_next):
    ...
    if (
        ...
        and not AI_WORKFLOW_JWT_PATH.match(request.url.path)
    ):
        try:
            verify_bearer(
