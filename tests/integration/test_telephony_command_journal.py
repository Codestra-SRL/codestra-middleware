    ...
    try:
        ...
        class SyntheticClient:
            ...
            async def dispatch(self, command_id, value, *, traceparent):
                ...
                return {
                    ...
                    "adapter_identity": "ENDP-SYNTHETIC-ADAPTER",
        ...
        async with factory() as session:
            ...
            terminal = terminal.model_copy(
                update={
                    "result_hash": payload_hash(
                        {
                            key: value
                            for key, value in terminal.model_dump(mode="json").items()
                            if key != "result_hash"
                        }
                    )
                }
            )
            created_result = await create_terminal_result(
                terminal, terminal.adapter_identity, session
            )
            duplicate_result = await create_terminal_result(
                terminal, terminal.adapter_identity, session
            )
            ...
            with pytest.raises(
                ...
                conflicting = terminal.model_copy(update={"readback_hash": "4" * 64})
                conflicting = conflicting.model_copy(
                    update={
                        "result_hash": payload_hash(
                            {
                                key: value
                                for key, value in conflicting.model_dump(
                                    mode="json"
                                ).items()
                                if key != "result_hash"
                            }
