...
def test_complete_inventory_is_required_for_available():
    result = audit_extension(6111, {"pjsip_endpoint": "ABSENT"})
    assert result.classification == ExtensionState.UNKNOWN_REQUIRES_REVIEW
    assert result.missing_sources


def test_configured_pool_candidate_only_available_with_every_authoritative_clear():
    result = audit_extension(6111, clear_evidence())
...
def test_global_exclusions_always_fail_closed():
    ...
    for extension in (6000, 6110, 6197, 6198):
        assert audit_extension(extension, clear_evidence()).classification == ExtensionState.EXCLUDED


def test_collision_precedence():
    evidence = clear_evidence()
    evidence["vicidial_phone"] = "PRESENT"
    assert audit_extension(6111, evidence).classification == ExtensionState.ASSIGNED
    evidence["asterisk_channel"] = "ACTIVE"
    assert audit_extension(6111, evidence).classification == ExtensionState.ACTIVE
...
def test_history_ambiguity_holds_extension():
    ...
    assert (
        audit_extension(6111, evidence).classification == ExtensionState.HISTORICAL_HOLD
    )
