...
def test_required_tables_and_tenant_uniqueness_are_migrated():
    ...
    required = {
        "customers", "locations", "contacts", "orders", "shipments", "loads", "routes", "stops",
        "drivers", "vehicles", "assignments", "status_events", "tracking_events", "rate_cards", "quotes",
        "quote_items", "charges", "documents", "proof_events", "exceptions", "claims", "notifications",
        "driver_settlements", "reconciliation", "audit_events",
    }
...
def test_no_browser_route_can_target_internal_services():
    ...
    for forbidden in ("vicidial", "asterisk", "qdrant", "127.0.0.1:11434", "n8n/webhook"):
...
def test_public_tracking_returns_only_minimized_fields():
    ...
    for forbidden in ("driver_external_key", "amount", "customer_external_key", "special_instructions"):
