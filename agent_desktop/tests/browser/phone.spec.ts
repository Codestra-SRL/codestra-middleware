...
test("requests bounded provisioning and fails closed when WSS is unavailable", async ({ page }) => {
  ...
  await page.goto("/");
  await page.getByRole("button", { name: "Provision + register" }).click();
  await expect.poll(() => requestBody).toMatchObject({ campaign_id: "TEST_SYN", endpoint: "6101" });
  await expect(page.getByText(/WebSocket closed/i)).toBeVisible();
  expect(await page.evaluate(() => ({ local: localStorage.length, session: sessionStorage.length, url: location.href }))).toEqual({
    local: 0, session: 0, url: "http://127.0.0.1:4173/",
  });
});
...
test("prevents duplicate control submission while connecting", async ({ page }) => {
  ...
  await page.goto("/");
  const button = page.getByRole("button", { name: "Provision + register" });
  await button.click();
...
test("authenticates realtime and renders one screen pop and recording state", async ({ page }) => {
  ...
  await page.goto("/");
  await page.evaluate(() => window.dispatchEvent(new CustomEvent("codestra:access-token", { detail: "synthetic-access-token" })));
