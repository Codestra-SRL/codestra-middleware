...
ALLOWED = {
    ...
    "cosign_key",
    "github_attestation",
    "digest_pin_plus_sbom_and_approved_risk_acceptance",
}
...
for item in policy["images"]:
    ...
    assert component["name"] == expected_subjects.get(
    ...
    if item["verification_method"] == "cosign_key":
        assert item["cosign_public_key_reference"]
    count = counts[item["image_name"]]
    high_risk = (
        count["trivy_critical"] + count["trivy_high"] + count["grype_high_or_critical"]
        > 0
    )
    if high_risk:
    ...
    for report in item["vulnerability_report_paths"]:
        report_path = (
            SECURITY / report if report.startswith("scans/") else REPOSITORY / report
        )
